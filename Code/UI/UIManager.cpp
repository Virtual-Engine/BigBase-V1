...
namespace Big::UserInterface
{
	...
	{
		static uint32_t* (*pRegisterFile)(int*, const char*, bool, const char*, bool) = reinterpret_cast<decltype(pRegisterFile)>(Memory::pattern("48 89 5C 24 ? 48 89 6C 24 ? 48 89 7C 24 ? 41 54 41 56 41 57 48 83 EC 50 48 8B EA 4C 8B FA 48 8B D9 4D 85 C9").count(1).get(0).get<void>(0));
		std::string path = g_Settings.appdatapath + "\\Textures\\" + FullHeaderName + ".ytd";
	...
	bool IsOnRect(float left, float top, float right, float bottom)
	{
		ImVec2 mouse_pos = ImGui::GetMousePos();
		Vector2 screen_res = g_Settings.screenResolution;
		int left_screen = static_cast<int>(left * screen_res.x);
		int top_screen = static_cast<int>(top * screen_res.y);
		int right_screen = static_cast<int>(right * screen_res.x);
		int bottom_screen = static_cast<int>(bottom * screen_res.y);

		bool is_mouse_in = mouse_pos.x >= left_screen && mouse_pos.x <= right_screen && mouse_pos.y >= top_screen && mouse_pos.y <= bottom_screen;
		return is_mouse_in;
	}
			...
			if (!m_SubmenuStack.empty())
			{
				...
				if (sub->GetNumOptions() != 0)
				{
					...
					for (std::size_t i = startPoint, j = 0; i < endPoint; ++i, ++j)
					{
						...
						{
							bool is_mouse_in_option = false;
							bool is_mouse_hovering_option = false;
							Vector2 screen_res;
							is_mouse_in_option = IsOnRect(m_PosX, m_DrawBaseY, m_OptionHeight, m_Width);
							if (IsMouseLocked() && is_mouse_in_option && !g_Settings.m_LockMouse && sub->GetSelectedOption() < m_OptionsPerPage)
							{
								int previous_option = sub->GetSelectedOption();
								is_mouse_hovering_option = true;
								float relativeMouseY = (GetMousePos().y / screen_res.y) - m_DrawBaseY;
								std::size_t claculatedSelectedOption = static_cast<std::size_t>(relativeMouseY / m_OptionHeight);

								sub->SetSelectedOption(claculatedSelectedOption);

								if (sub->GetSelectedOption() != previous_option)
								{

								}
							}
							else if(IsMouseLocked() && is_mouse_in_option && !g_Settings.m_LockMouse)
							{
								is_mouse_hovering_option = true;
							}
							else
							{
								is_mouse_hovering_option = false;
							}
								DrawRect(
									m_PosX,
									tmp_point,
	...
	void UIManager::DrawOption(AbstractOption* opt, AbstractSubmenu* sub, bool selected)
	{
			...
			if (opt->GetFlag(OptionFlag::BoolOption))
			{
				...
				else if (ToggleIterator == 1)
				{
					if (ToggledOn)
					{
						GRAPHICS::DRAW_SPRITE("commonmenu", "shop_tick_icon",
							g_UiManager->m_PosX + (g_UiManager->m_Width / g_UiManager->m_OptionPadding - 0.005f)
							, g_UiManager->m_DrawBaseY + (g_UiManager->m_OptionHeight / 2.f) - (g_UiManager->GetTextHeight(g_UiManager->m_OptionFont
								, g_UiManager->m_OptionTextSize) / 1.5f) + 0.014f
							, res.x, res.y, 0.0, m_SelectedToggle.r, m_SelectedToggle.g, m_SelectedToggle.b, m_SelectedToggle.a, 1);
